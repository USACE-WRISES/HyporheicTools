...
# ----------------------------
# Step 4 â€“ Model boundaries & idomain
# ----------------------------
...
# ----------------------------
# NEW â€” WSE edge/endpoint helpers (valid WSE only)
# ----------------------------
...
def build_wse_valid_edge_index(
    ...
    *,
    ...
    with rasterio.open(wse_raster_path) as src:
        ...
        # --- Build boolean valid mask (north-up or rotatedâ€”doesn't matter)
        ...
        # --- Edge pixel centers (+ values) â€” unchanged behavior
...
def csv_points_elevation(points_gdf: gpd.GeoDataFrame,
    ...
    if out_csv is not None:
        ...
        print(f"Elevation CSV written â†’ {out_csv}")
...
def compile_chd_data(
    ...
    def _add(cells, heads):
        for idx, (k, j, i) in enumerate(cells):
            if (k, j, i) not in unique:
                chd_data.append([k, j, i, float(heads[idx])])
                unique.add((k, j, i))
            else:
                dupes.add((k, j, i))

    _add(left_cells_0, left_heads)
    _add(right_cells_0, right_heads)
    _add(up_cells_0, up_heads)
    _add(down_cells_0, down_heads)

    return chd_data, len(unique), len(dupes)


# ----------------------------
# Step 6 â€“ Build & run models
# ----------------------------
def build_gwf_model(cfg, chd_data: Sequence[Sequence[float]], idomain: np.ndarray) -> tuple[flopy.mf6.MFSimulation, flopy.mf6.ModflowGwf]:
    ...
    * If KH/KV polygons are not provided, uniform cfg.kh/cfg.kv are expanded to full
    ...
    # DIS: top/botm/idomain â†’ binary (as you had)
    if dis is not None:
        dis.top.set_record({
    ...
    # IC: strt â†’ binary (as you had)
    if ic is not None:
        ic.strt.set_record(_layered_records(ic.strt.array, "strt"))

    # NPF: NEW â€” write k and k33 per layer as binary
...
# =========================
# NEW â€” Hydraulic head exports (netCDF + per-layer rasters + mosaic)
# =========================
def export_hydraulic_head_layers(*, cfg, gwf, log=print) -> dict:
    ...
    try:
        ...
        print(f"Wrote {len(geotiffs)} head GeoTIFFs â†’ {tifs_dir}")
    except Exception as e:
...
"""
            ...
            print(f"Built mosaic dataset â†’ {md_path}")
    ...
    if can_write_netcdf:
        try:
            ...
            print(f"Wrote netCDF head cube â†’ {nc_path}")
        except Exception as e:
...
def process_and_export_modpath7_results(
    ...
    *,
    ...
    """
    Export full MODPATH7 results with correct columns and CRS handling and write figures.
    Adds true 3D pathlines (POLYLINE Z) export while preserving all attributes.
    """
    ...
    if projection_file is not None:
        prj_path = Path(projection_file)
        if not prj_path.exists():
            raise FileNotFoundError(f"projection_file not found: {prj_path}")
    ...
    # --- robust origin extraction (handles MFScalar) ---
    xorigin = coerce_to_float(getattr(dis, "xorigin", xorigin_value if xorigin_value is not None else 0.0),
                              default=(xorigin_value if xorigin_value is not None else 0.0))
    yorigin = coerce_to_float(getattr(dis, "yorigin", yorigin_value if yorigin_value is not None else 0.0),
    ...
    df_pl = pd.DataFrame.from_records(pathrecs, columns=cols)
    if df_pl.empty:
    ...
    y_local = total_y - y_local_top  # bottom-up

    x_abs = xorigin + (x_local * cos_t - y_local * sin_t)
    y_abs = yorigin + (x_local * sin_t + y_local * cos_t)
    ...
    x_local_f = df_long["x"].to_numpy(dtype=float)
    y_topdown = df_long["y"].to_numpy(dtype=float)
    total_y = float(y_edges[-1])
    y_bottom = total_y - y_topdown

    cols_idx = np.searchsorted(x_edges, x_local_f, side="right") - 1
    rows_idx = np.searchsorted(y_edges, y_bottom, side="right") - 1

    ncol_mg = gwf.modelgrid.ncol
    nrow_mg = gwf.modelgrid.nrow
    oob = (cols_idx < 0) | (cols_idx >= ncol_mg) | (rows_idx < 0) | (rows_idx >= nrow_mg)
    cols_idx = cols_idx.astype(float)
    cols_idx[oob] = np.nan
    rows_idx = rows_idx.astype(float)
    rows_idx[oob] = np.nan
    ...
    for start in range(0, valid_idx.size, CHUNK):
        sel = valid_idx[start:start + CHUNK]
    ...
    for kk in range(1, nlay):
        cell_volumes[kk, :, :] = delr_mean * delc_mean * (float(z) if z is not None else 0.5)
    ...
    df_particle_summary = (
        total_volume_available_along_path
    ...
    df_particle_summary["z_cell_size_ft"] = float(z) if z is not None else 0.5
    ...
    artifacts = {
        ...
        "points_shp": None,
        "lines_shp": None,           # 2D
        "lines_shp_3d": None,        # 3D shapefile (preferred)
        "lines_fc_3d": None,         # 3D FGDB feature class (if created)
    ...
    # ----------------------- SPATIAL EXPORTS -----------------------
    spatial_requested = any([export_shp, export_shp_wgs84, export_kml, export_kmz, export_gpkg, export_shp_3d])
    if spatial_requested:
        # Points (2D, per cell) as before
        ...
        # Pathlines 2D (kept for 2D workflows)
        line_rows_2d = []
        line_geoms_2d = []
        for pid, g in (df_long
                       .sort_values(["particleid", "time"])
                       .groupby("particleid", sort=False)):
        ...
        for pid, g in (df_long
            ...
            if valid.sum() > 1:
                pts3d = list(zip(xs[valid], ys[valid], zs[valid]))
                line_geoms_3d.append(LineString(pts3d))
        ...
        # ---- Write with GeoPandas/Fiona (2D + 3D Shapefile fallback) ----
        if export_shp:
        ...
        # Try a 3D shapefile via GeoPandas as a fallback (some GDAL builds preserve Z automatically)
        gpd_3d_path = None
        if export_shp_3d:
            try:
                shp3d = output_folder / f"{direction}_hyporheic_pathlines_3D_HECRAS_CRS.shp"
                lines_gdf_3d.to_file(shp3d, driver="ESRI Shapefile")
                gpd_3d_path = str(shp3d)
                artifacts["lines_shp_3d"] = gpd_3d_path
            except Exception as e:
                print(f"[WARN] GeoPandas 3D shapefile export failed, will try ArcPy route if available. ({e})")

        # ---- Prefer ArcPy POLYLINE Z shapefile (robust in Pro) ----
        if export_shp_3d:
            try:
                import arcpy
                # Spatial reference from hec_crs (try EPSG first)
                sr = None
                try:
                    epsg = hec_crs.to_epsg() if hasattr(hec_crs, "to_epsg") else None
                    if epsg:
                ...
                if sr is None:
                    try:
                        wkt = hec_crs.to_wkt() if hasattr(hec_crs, "to_wkt") else str(hec_crs)
                        sr = arcpy.SpatialReference()
                        sr.loadFromString(wkt)
                    except Exception:
                        sr = None

                out_shp = output_folder / f"{direction}_hyporheic_pathlines_3D.shp"
                ...
                # Add short-form fields (match your 2D attributes)
                field_types = {}
                for fld in df_particle_summary_short.columns:
                    if fld == "particleid":
                        arcpy.management.AddField(str(out_shp), fld, "LONG")
                        field_types[fld] = int
                    else:
                        arcpy.management.AddField(str(out_shp), fld, "DOUBLE")
                        field_types[fld] = float

                # Build particle â†’ attributes lookup
                attr_lut = df_particle_summary_short.set_index("particleid").to_dict("index")

                # Insert geometries
                use_fields = ["SHAPE@"] + list(df_particle_summary_short.columns)
                with arcpy.da.InsertCursor(str(out_shp), use_fields) as cur:
                    for pid, g in (df_long
                                   .sort_values(["particleid", "time"])
                                   .groupby("particleid", sort=False)):
                        xs = g["x_abs"].to_numpy(float)
                        ...
                        if valid.sum() < 2:
                            continue
                        arr = arcpy.Array([arcpy.Point(float(x), float(y), float(z)) for x, y, z in zip(xs[valid], ys[valid], zs[valid])])
                        geom = arcpy.Polyline(arr, sr, has_z=True)
                        attrs = attr_lut.get(int(pid), {})
                        row_vals = [geom] + [attrs.get(f, None) for f in df_particle_summary_short.columns]
                        cur.insertRow(row_vals)

                artifacts["lines_shp_3d"] = str(out_shp)  # prefer ArcPy-created path
            except Exception as e:
                if not gpd_3d_path:
                    print(f"[WARN] ArcPy 3D shapefile export failed and no GeoPandas 3D file exists. ({e})")
        ...
        # Optional KML/KMZ + GPKG (unchanged)
        if export_kml or export_kmz:
            ...
            if export_kmz:
                if not export_kml:
                    tmp_points_kml = output_folder / f"._tmp_{direction}_hyporheic_points.kml"
                    tmp_lines_kml = output_folder / f"._tmp_{direction}_hyporheic_pathlines.kml"
                    points_wgs.to_file(tmp_points_kml, driver="KML")
                    lines_wgs.to_file(tmp_lines_kml, driver="KML")
                    created_temp = True
                ...
                with zipfile.ZipFile(p_kmz, "w", zipfile.ZIP_DEFLATED) as kmz:
                    kmz.write(tmp_points_kml, tmp_points_kml.name)
                with zipfile.ZipFile(l_kmz, "w", zipfile.ZIP_DEFLATED) as kmz:
                    kmz.write(tmp_lines_kml, tmp_lines_kml.name)
                artifacts["points_kmz"] = str(p_kmz)
                artifacts["lines_kmz"] = str(l_kmz)

                if created_temp:
                    try:
                        tmp_points_kml.unlink(missing_ok=True)
                        tmp_lines_kml.unlink(missing_ok=True)
                    except Exception:
                        pass
    ...
    if export_results_txt:
        ...
        if not results_txt.exists():
            results_txt.write_text("Stats generated with df_prc and df_particle_summary (full columns).\n")
        artifacts["results"] = str(results_txt)

    # (plots unchanged)
    if include_pngs_in_return and not export_pngs:
        artifacts["plots"] = [
            f"{direction}_start_end_points.png",
            f"{direction}_pathlines_endpoints.png",
            f"filtered_{direction}_pathlines_endpoints.png",
            f"{direction}_pathlines_3d.png",
            f"{direction}_pathline_length_width_depth_distributions.png",
            f"{direction}_residence_time_distribution.png",
            f"{direction}_velocity_distribution.png",
            "top_layer_cell_volumes.png",
            f"{direction}_hyporheic_space_2d_map.png",
        ]
    ...
    *,
    ...
    for pid in all_pids:
        try:
            ...
            # Itâ€™s ok if some particles have no pathline records
            continue
...
def plot_hyporheic_workflow(
    *,
    ...
    """
    Generate the figure set (1â€“5) and optionally export GeoTIFFs + start/end SHPs/CSVs.
    """
    ...
    print(f"Model-axis fraction inside [0..Î£delr]Ã—[0..Î£delc]: {report.get('model_axis_fraction_inside_rect_0_total')}")
    print("=====================================\n")

    out["alignment_debug_report"] = report

    # ---------- FIGURE 1 â€” Head overlay ----------
    ...
    # ---------- FIGURE 2 â€” Overlay + pathlines/points ----------
    fig, ax = plt.subplots(figsize=(12, 10))
    ax.imshow(img, extent=extent, origin="upper", zorder=1)
    contourf = ax.contourf(Xc, Yc, head_layer_clipped, levels=levels, cmap="viridis", alpha=1.0, zorder=0)
    ...
    try:
        ...
        ax.set_zlabel(f"Elevation (ft)  (VE Ã—{ve_iso:g})")
        ax.set_title(f"Isometric View â€” VE Ã—{ve_iso:g}")
    ...
    try:
        ...
        ax.set_zlabel(f"Elevation (ft)  (VE Ã—{ve_long:g})")
        ax.set_title(f"Longitudinal View â€” VE Ã—{ve_long:g}")
...
def export_fig45_diagnostics(
    *,
    ...
    try:
        ...
        print(f"[Diagnostics/{direction}] World grid extent: [{xmin:.2f}, {ymin:.2f}] â€“ [{xmax:.2f}, {ymax:.2f}]")
        print(f"[Diagnostics/{direction}] Modelâ€‘axis extent X: 0 â€“ {np.sum(delr_arr):.2f}; Y: 0 â€“ {np.sum(delc_arr):.2f}")
    except Exception:
        pass
